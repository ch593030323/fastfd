...
KindIndexWidget::KindIndexWidget(QWidget *parent) :
...
KindIndexWidget::~KindIndexWidget()
{

}

void KindIndexWidget::preSort()
{
    if(index <= 0)
        return;
    --index;
...
void KindIndexWidget::clearSelected()
{
    if(!_lastItem.isNull())
        _lastItem->setSelect(false);
}

void KindIndexWidget::showSort()
{
...
void KindIndexWidget::addSort(QString kind, QString name)
{
    kindList.append(kind);
    nameList.append(name);//大类的名字
}

void KindIndexWidget::itemClicked(QString kindId)
{
    if(kindId.isEmpty())
    {
        return;
