      ...
      ui(new Ui_S_Mainwindow)
{
    ...
    ui->toolBar->addWidget(fsline = new QLineEdit);

    ui->toolBar->addWidget(istext = new QCheckBox(tr("打印文本")));
    ...
    connect(fsline,SIGNAL(editingFinished(QString)),this,SLOT(setXywhInfo()));
...
void S_MainWindow::addShape()
{
    ...
    dialog.setup(headerlabels); qDebug() << headerlabels;
...
void S_MainWindow::updateXywhInfo(const QRect &rect)
{
    ...
    fsline->setText(shape._fontsize);
...
void S_MainWindow::setXywhInfo()
{
    ...
    doc->setShapefontsize(shapeName, fsline->text());
...
void S_MainWindow::initDocument(const SSRPairMap &ssrmap)
{
    ...
    Shape newShape(Shape::Rectangle, randomColor()
                   , QRect(0,0,w / _factor  + min_off,h / _factor  + min_off), "BACKGROUND");
    ...
    foreach(QString name, names){
        ...
        Shape newShape(type, randomColor()
                       ...
                       , name
                       , _ssrmap[name].value(2).toString()
                       , true
                       , _ssrmap[name].value(3).toBool());
...
void S_MainWindow::addtwobarcode()
{
    ...
    Shape newShape(Shape::TwoBarCode, randomColor(), randomRect(doc->size()));
    newShape._text = key;
    doc->undoStack()->push(new AddShapeCommand(doc, newShape));
}
...
void S_MainWindow::addonebarcode()
{
    ...
    Shape newShape(Shape::OneBarCode, randomColor(), randomRect(doc->size()));
    newShape._text = key;
    doc->undoStack()->push(new AddShapeCommand(doc, newShape));
}
...
SSRPairMap S_MainWindow::printMap()
{
    ...
    foreach(Shape shape ,shapes){
        ...
        map.insert(shape._text,QVariantList() << (int)shape.type()
