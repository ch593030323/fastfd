...
{
    QByteArray b0 = byte2numbyte(trans_u2g(content),maxsize);
    QByteArray b = "F=";
...
QMap<int, QString> ElectronicScale_CAS::WeigetType()
{
    if(_wtype.isEmpty()){
        _wtype.insert(1, QObject::tr("称重"));
        _wtype.insert(2, QObject::tr("数量"));
        _wtype.insert(3, QObject::tr("个"));
    }
...
{
    QStringList() << QObject::tr("上海凯士 CL5200");
}
