...
QByteArray ElectronicScale_DAHUA::downloadPLUINFO(QStandardItemModel *model, int row)
{
    ...
    for(int c = 0; c < 26; c++){
        ...
        // qDebug() << model->item(row, c)->text() << c;
    }
    s0 += "B";
    s0 += hanzi2quweima(model->item(row, 26)->text());//qDebug() << model->item(row, 26)->text();
    s0 += "C";
    s0 += hanzi2quweima(model->item(row, 27)->text());//qDebug() << model->item(row, 27)->text();
    s0 += "D";
    s0 += hanzi2quweima(model->item(row, 28)->text());//qDebug() << model->item(row, 28)->text();
    s0 += "E";
    //    qDebug() << s0;

    return QByteArray().append(s0) + QByteArray::fromHex("0D0A");
}
...
QMap<int, QString> ElectronicScale_DAHUA::WeigetType()
{
    if(_wtype.isEmpty()){
        _wtype.insert(0, QObject::tr("称重"));
        _wtype.insert(1, QObject::tr("计件"));
        _wtype.insert(2, QObject::tr("定重"));
    }
