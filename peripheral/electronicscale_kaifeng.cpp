...
QMap<int, QString> ElectronicScale_KAIFENG::WeigetType()
{
    if(_wtype.isEmpty()){
        _wtype.insert(0, QObject::tr("称重"));
        _wtype.insert(1, QObject::tr("定价"));
        _wtype.insert(2, QObject::tr("定重定价"));
    }
    return _wtype;
}
...
QByteArray ElectronicScale_KAIFENG::downloadPLUINFO(const QString &plu, const QString &goodsname, const QString &barcode, const QString &price, const QString &wtype, const QString &validate)
{
    //! 生成data
    QByteArray b;
    char name[] =// 商品名称 以gbk为标准；转换后的指令需要左对齐；长度0～12个汉字；
    ...
    b += /*name*/ _alignLeft_fixedLen_byte2byte(trans_u2g(goodsname), 24, char(0))+
            /*plucode*/_alignRight_fixedLen_int2bcd(barcode, 6, char(0))+
            QByteArray(padding,sizeof(padding));
    {
        ...
        p2 = _alignRight_fixedLen_byte2byte(p2,4,char(0));
        foreach(char c, p2)
            p3.push_front(c);
        b += p3;
    }
    ...
    b += QByteArray(weight,sizeof(weight))+QByteArray(discount,sizeof(discount))+
            /*pluno*/_alignRight_fixedLen_int2bcd(plu,3, char(0))+
    ...
    char datatype[] =//0 4都没有打印
            /*1*/{0x04};
    b += /*validate0*/_alignRight_fixedLen_int2hex(validate,2,char(0))
            +QByteArray(tare,sizeof(tare))+
            /*type*/_alignRight_fixedLen_int2hex(wtype,1,char(0)) +QByteArray(datatype,sizeof(datatype));
    ...
    /*2*/char stx[] = {0x03, 0xff};
    /*2*/char command[] = {0x43,0xff};
    ...
    part1 = QByteArray(stx,sizeof(stx)) + QByteArray(command,sizeof(command)) + QByteArray(paramete,sizeof(paramete)) + QByteArray(ecr_id,sizeof(ecr_id)) +
            QByteArray(length,sizeof(length));
    /*2*/char crc[2]; crc[0] = _2xor(part1); crc[1] = 0;

    /*n*/QByteArray &data = ba_data;
    /*2*/char crc_2[2]; crc_2[0] = _2xor(data); crc_2[1] = 0;
...
QByteArray ElectronicScale_KAIFENG::setShopName(const QString &shop_name)
{
    ...
    QByteArray ba_data =
            ...
            _alignLeft_fixedLen_byte2byte(trans_u2g(shop_name),25, char(0)) +
            QByteArray(padding,sizeof(padding));
    int data_length = ba_data.length();
    QByteArray part1,part2,ins;

    /*2*/char stx[] = {0x03, 0xff};
    /*2*/char command[] = {0x46,0xff};
    /*4*/char paramete[]={0x00,0x00,0x00,0x00};
...
{
    char c[] = {0x03,0xff,0x41,0xff};
    return QByteArray(c, sizeof(c));
}

QByteArray ElectronicScale_KAIFENG::getSysParam()
{
    char c[] = {0x03,0xff,0x81,0xff};
    return QByteArray(c, sizeof(c));
}
...
{
    char c[] = {0x03,0xff,0x30,0x00,0x00,0x00,0x00,0x00,
