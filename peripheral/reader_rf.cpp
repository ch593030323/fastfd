...
static const char LOADKEY_MODE= 0x00;//0,0套密码,1,1套密码,2,2套密码,3,3不允许
static const char AUTHENTICATION_MODE= 0x00;//0,0套密码,1,1套密码 ,2,2套密码,3,MCM-ROM密码验证
static const char BEEPTIME= 0x05;//蜂鸣时长
static const char replenish[]={0x1a, 0x2f, 0x45, 0x64, 0x8e, 0x9a, 0xa1, 0xb5, 0xf6, 0x7e, 0xf9, 0xbf, 0x91, 0x34, 0x67, 0x80};
//有效传输密码和用户密码为6字节， 但通讯按8 字节发送， 最后两个字节为任意值。
static const char TKEY[] ={0x7d, 0x3e, 0x9f, 0x4f, 0x95, 0x95, 0x60, 0xf7};//传输密码
//static const char NKEY[] ={0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6};//下载到RAM中的密码
static const char NKEY[] ={0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
static  char SECNR=0x01;//扇区 android 1； msmk 2； 超市 3； qt 餐饮 4
static const  char ADDRESS[]={SECNR*4,SECNR*4 + 1,SECNR*4 + 2,SECNR*4 + 3};//块地址
...
bool Reader_RF::open()
{
    ...
    //qDebug("libusb_kernel_driver_active = %d\n", r);//0 is the interface to check
    ...
    if((r=libusb_claim_interface(Curdev,0)) < 0){
        //qDebug("libusb_claim_interface = %d\n", r);
        isopen = false;
        return isopen;
    }
    //一般错误
    read_card_error=
            QStringList()
            <<tr("函数调用成功")<<tr("在操作区域内无卡")<<tr("")<<tr("")
           <<tr("验证不成功")<<tr("")<<tr("通讯错误")<<tr("")<<tr("读系列号错误")<<tr("")<<tr("卡片没有通过验证");
    //默认密码
...
bool Reader_RF::readcard(QString &part0, QString &part1, QString &part2)
{
...

    {
        ...
        if((r = getRespone(Curdev, tmp46)) < 0 ){
            //qDebug() << "getRespone(Curdev,tmp46) = " << libusb_error_name(r);
            otherError = libusb_error_name(r);
...
bool Reader_RF::beep()
{
    ...
    int r=libusb_control_transfer(Curdev, 0x21, 0x09, 0x306, 0x0, (unsigned char *)array.data(), 0xff, 1000);
    if(r < 0){
        otherError = libusb_error_name(r);
        return false;
    }
...
{    
    ...
    if(s >= 0 && s < read_card_error.size())
        err =  read_card_error.value(s);
#endif
...
int Reader_RF::getRespone(libusb_device_handle *connection, const QByteArray &tmp)
{
    qDebug() << "-----------------------------------------------------------------------" << tmp.toHex();
    int r;
    r = libusb_control_transfer(connection, 0x21, 0x09, 0x306, 0x00, (unsigned char *)tmp.data(), 0xff, 1000);
    if(r < 0){
        otherError = libusb_error_name(r);
    }

    r = libusb_control_transfer(connection, 0xa1, 0x01, 0x306, 0x00, (unsigned char *)byte2.data(), 0xff, 1000);
    if(r < 0){
        otherError = libusb_error_name(r);
    }
...
void Reader_RF::printdev(libusb_device *dev) {
    ...
    qDebug()<<"VendorID: "<<QString().sprintf("0x%x", desc.idVendor);
    qDebug()<<"ProductID: "<<QString().sprintf("0x%x", desc.idProduct);
...
QString Reader_RF::simpleError() const
{
    int s = QByteArray().append(byte2[3]).toHex().toInt();
    if(s >= 0 && s < read_card_error.size())
        return read_card_error.value(s);
    return "unknow error";
}
