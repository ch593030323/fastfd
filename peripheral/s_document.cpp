...
Shape::Shape(Type type, const QColor &color, const QRect &rect, const QString &text, const QString &fsize, bool ismoving, bool istext)
    : m_type(type), m_rect(rect), m_color(color),_text(text),_fontsize(fsize), _ismoving(ismoving),_istext(istext)
{
}
...
Shape::Type Shape::stringToType(const QString &s, bool *ok)
{
    ...
    if (s == QLatin1String("Triangle"))
        return Triangle;
    if (s == QLatin1String("twobarcode"))
        return TwoBarCode;
    if (s == QLatin1String("onebarcode"))
        return OneBarCode;
...
int S_Document::indexAt(const QPoint &pos) const
{
    for (int i = m_shapeList.count() - 1; i >= 0; --i) {
        if (m_shapeList.value(i).rect().contains(pos))
            return i;
    }
    return -1;
}
...
void S_Document::setCurrentShape(int index)
{
    ...
    if (m_currentIndex != -1)
        update(m_shapeList.value(m_currentIndex).rect());
...
{
    for (int i = 0; i < m_shapeList.count(); ++i) {
        if (m_shapeList.value(i).name() == shapeName)
            return i;
    }
    return -1;
}
