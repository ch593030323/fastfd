...
/*
    ...
    s0 += _t("0") +
            ...
            _t("0") +
            _t("9") +
            _t_t(_t_t(_t_t(_t(hanzi2quweima("水果")))));//设置商品名 5477
...
QByteArray ElectronicScale_YSHENG::downloadPLUINFO(QStandardItemModel *model, int row)
{
    ...
    /*称重方式*/QByteArray weigettype = QByteArray().append(model->item(row, 3)->text());
    /*单价*/QByteArray price = QByteArray().append(_price(model->item(row, 2)->text()));
    /*商品名称*/QByteArray barcodename = trans_u2g(model->item(row, 26)->text());
    ...
    /*商店名称*/QByteArray shopname = trans_u2g(model->item(row, 27)->text());

    //    商品信息
    s0 += _tx1("DWL") + _tx1("PLU") + "\n";
    s0 += _tx1("PLU") +_tx1(plu) + _tx2(barcode) +  _tx1(weigettype) +  _tx1(price);//单价(元/kg)        -1 +2 + 4
...
QMap<int, QString> ElectronicScale_YSHENG::WeigetType()
{
    if(_wtype.isEmpty()){
        _wtype.insert(1, QObject::tr("计重"));
        _wtype.insert(2, QObject::tr("计件"));
...
QStringList ElectronicScale_YSHENG::BarType()
{
    ...
    if(_btype.isEmpty()){
        _btype << QObject::tr("BBEEEEELLLLLC");
...
{
    //1.3kg -> 130
    QString _data = QString().sprintf("%.02f", data.toFloat() / 100);
